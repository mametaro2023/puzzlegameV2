...
export class Renderer {
    ...
    drawInventory(board, animationManager, now) {
        ...
        if (animationManager.usedItemAnimation) {
            const p = Math.min((now - animationManager.usedItemAnimation .startTime) / animationManager.usedItemAnimation .duration, 1.0);
            const scale = 1 + p * 0.5;
            const alpha = 1 - p;
            ...
            this.ctx.textAlign = 'center';
            this.ctx.textBaseline = 'middle';
            this.ctx.fillText(animationManager.usedItemAnimation .item, 0, 0);
            this.ctx.restore();
        }
        ...
        for (let i = 0; i < board.inventory.length; i++) {
            ...
            if (animationManager.inventorySlideAnimation) {
                const p = Math.min((now - animationManager.inventorySlideAnimation.startTime) / animationManager.inventorySlideAnimation.duration, 1.0);
