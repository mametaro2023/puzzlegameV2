...
import * as C from './config.js'; // LOCK_DELAY を使うためにインポート
...
export class GameController {
    constructor(canvas, ctx) {
        ...
        this.lockTimer = null; // ロックタイマーをコントローラーで管理
        ...
        this.player1Board.onGaugeMax(() => {
            ...
            const gaugeAnimInfo = this.player1Board.setGauge(0); // 攻撃後は0に
    ...
    loop() {
        ...
        if (board.cur && !board.falling && !this.lockTimer) {
            // ミノが接地していて、まだタイマーが動いていないなら、タイマーを開始
            this.lockTimer = setTimeout(() => this.lockPiece(), C.LOCK_DELAY);
        } else if (board.cur && board.falling && this.lockTimer) {
            // 空中に浮いた（落下が再開した）なら、タイマーを解除
    ...
    hardDrop() {
        clearTimeout(this.lockTimer); // タイマーをキャンセル
        this.lockTimer = null;
    ...
    handleLockResult(lockResult) {
        ...
        // ゲージアニメーションがあれば開始
        if (lockResult.gaugeAnimInfo) {
            this.handleAnimationInfo(lockResult.gaugeAnimInfo);
        }
        // 消えたブロックがあればパーティクルを生成
        ...
        // 落下するブロックがあれば落下アニメーションを開始
        if (lockResult.fallingBlocks.length > 0) {
            // animStartTime をここでセットするのが重要
            const now = performance.now();
            this.animationManager.fallingBlocks = lockResult.fallingBlocks.map(b => ({
                ...b,
                animStartTime: now,
                easing: 'easeOutBounce'
            }));
            
            // 落下アニメーションが終わる頃に、再度 startClear を呼び出して連鎖をチェック
            setTimeout(() => {
                this.animationManager.fallingBlocks = []; // 古いアニメーション情報をクリア
                this.handleLockResult(this.player1Board.startClear());
            }, C.FALL_ANIM_DURATION + 50); // アニメーション時間 + 猶予
        }
        
        // 新しいミノを出現させる必要がある場合
        if (lockResult.needsSpawn) {
            this.player1Board.spawn();
            // NEXTミノのアニメーションをトリガー
            this.animationManager.triggerNextMinoAnimation(
                this.player1Board.nextQueue[0],
                this.player1Board.nextQueue[1]
            );
        }
    }
